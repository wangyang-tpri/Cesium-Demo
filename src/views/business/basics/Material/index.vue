...
function polyPositions() {
  return Cesium.Cartesian3.fromDegreesArray([
    108.935, 34.338, 108.945, 34.338, 108.945, 34.345, 108.935, 34.345,
  ]);
}
      ...
      new Cesium.Entity({
        polygon: {
          ...
          material: new Cesium.StripeMaterialProperty({
            evenColor:
              Cesium.Color.fromCssColorString("#48dbfb").withAlpha(0.85),
            oddColor:
              Cesium.Color.fromCssColorString("#0984e3").withAlpha(0.85),
            repeat: 6, // 条纹重复次数
          }),
        },
      })
...
function linePositions() {
  return Cesium.Cartesian3.fromDegreesArray([
    108.94, 34.33, 108.95, 34.335, 108.96, 34.33,
  ]);
}
...
const explainMap: Record<string, () => string> = {
  color:
    () => `【原理】Entity 图形的 material 属性接收 MaterialProperty（随时间可变的材质）。
...
lineCount 控制网格密度。适合平面统计图、棋盘格底图。`,
  ...
  image:
    () => `【原理】ImageMaterialProperty 将任意图片（URL/DataURI/Canvas）作为贴图，
...
<template>
  <div class="flex h-full flex-col gap-3 p-4">
    <div class="flex flex-wrap items-center gap-2">
      ...
      <n-button
        size="small"
        :type="activeFeature === 'stripe' ? 'primary' : 'default'"
        @click="applyStripe"
        >条纹 Stripe</n-button
      >
      <n-button
        size="small"
        :type="activeFeature === 'checker' ? 'primary' : 'default'"
        @click="applyChecker"
        >棋盘格</n-button
      >
      <n-button
        size="small"
        :type="activeFeature === 'grid' ? 'primary' : 'default'"
        @click="applyGrid"
        >网格 Grid</n-button
      >
      <n-button
        size="small"
        :type="activeFeature === 'image' ? 'primary' : 'default'"
        @click="applyImage"
        >图片 Image</n-button
      >
      <n-button
        size="small"
        :type="activeFeature === 'glow' ? 'primary' : 'default'"
        @click="applyGlow"
        >发光 Glow</n-button
      >
      <n-button
      ...
      <n-button
        size="small"
        :type="activeFeature === 'dash' ? 'primary' : 'default'"
        @click="applyDash"
        >虚线 Dash</n-button
      >
