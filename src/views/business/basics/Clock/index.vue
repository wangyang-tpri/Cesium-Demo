...
const statusText = ref('时钟已初始化：模拟时间从 00:00 到 24:00，1 秒 = 60 秒模拟时间');
...
function ensureOrbit() {
  ...
  orbitEntity = v.entities.add({
    ...
    position: new Cesium.CallbackProperty((time) => {
      ...
      const t = Cesium.JulianDate.secondsDifference(time ?? Cesium.JulianDate.now(), start) / 86400; // 0~1
...
function applyAnimate() {
  ...
  statusText.value = v.clock.shouldAnimate ? '▶ 动画播放中（卫星沿轨道运动）' : '⏸ 动画已暂停';
}
...
function applySpeed() {
  ...
  statusText.value = `倍速：${SPEED_LABELS[speedIndex.value]}（multiplier = ${SPEEDS[speedIndex.value]!}）`;
}
...
function applyShowClockInfo() {
  ...
  statusText.value = `当前模拟时间：${iso}（multiplier=${v.clock.multiplier}，${v.clock.shouldAnimate ? '播放中' : '暂停'}`;
}
...
<template>
  <div class="flex h-full flex-col gap-3 p-4">
    <div class="flex flex-wrap items-center gap-2">
      <n-button size="small" :type="activeFeature === 'animate' ? 'primary' : 'default'" @click="applyAnimate">播放 / 暂停</n-button>
      <n-button size="small" :type="activeFeature === 'speed' ? 'primary' : 'default'" @click="applySpeed">倍速：{{ SPEED_LABELS[speedIndex] }}</n-button>
      <n-button size="small" :type="activeFeature === 'clock-basic' ? 'primary' : 'default'" @click="applyTimeJump">跳到 12:00</n-button>
      <n-button size="small" :type="activeFeature === 'clock-basic' ? 'primary' : 'default'" @click="applyShowClockInfo">读取模拟时间</n-button>
      <n-button size="small" :type="activeFeature === 'dynamic' ? 'primary' : 'default'" @click="activeFeature = 'dynamic'; ensureOrbit()">查看时间驱动实体</n-button>
    </div>
