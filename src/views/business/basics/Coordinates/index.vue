...
onMounted(() => {
  ...
  clickHandler.setInputAction(
    (e: Cesium.ScreenSpaceEventHandler.PositionedEvent) => {
      ...
      statusText.value = `点击点：经度 ${lon.toFixed(4)}°，纬度 ${lat.toFixed(
        4
      )}°，高度 ${height.toFixed(0)} m`;
    },
...
function applyDistance() {
  ...
  statusText.value =
    `北京→西安：直线距离 ${straightKm.toFixed(0)} km，` +
    `地表大圆距离 ${surfaceKm.toFixed(0)} km`;
}
...
function applyWorldToScreen() {
  ...
  if (windowPos) {
    statusText.value = `世界坐标(108.94,34.341) → 屏幕坐标 (${windowPos.x.toFixed(
      0
    )}, ${windowPos.y.toFixed(0)}) px`;
  } else {
...
function onFeatureClick(feature: string) {
  ...
  if (feature === "deg2cart") {
    ...
    statusText.value =
      `Cartesian3(x=${cartesian.x.toFixed(2)}, y=${cartesian.y.toFixed(
        2
      )}, z=${cartesian.z.toFixed(2)}) → ` +
      `弧度(lon=${carto.longitude.toFixed(6)}, lat=${carto.latitude.toFixed(
        6
      )}) → ` +
      `角度(lon=${lonDeg.toFixed(4)}°, lat=${latDeg.toFixed(
        4
      )}°, height=${carto.height.toFixed(2)}m)`;
  } else if (feature === "screen2world") {
    statusText.value =
      "点击地球任意位置，体验“屏幕坐标 → 地理坐标”转换（结果显示在落点标注和此处）";
  }
}
...
const explainMap: Record<string, () => string> = {
  deg2cart:
    () => `【原理】Cesium 内部统一使用地固系直角坐标 Cartesian3（x/y/z，单位米，
...
反之 toRadians() 用于把角度喂给相机/朝向。`,
  ...
  screen2world:
    () => `【原理】camera.pickEllipsoid 从相机出发，向屏幕点方向发射射线，
...
【结果】北京→西安：直线 ≈ 939 km，地表大圆 ≈ 940 km（差异来自地球曲率）。`,
  ...
  world2screen:
    () => `【原理】wgs84ToWindowCoordinates 是 pickEllipsoid 的逆过程：
...
<template>
  <div class="flex h-full flex-col gap-3 p-4">
    <div class="flex flex-wrap items-center gap-2">
      <n-button
        size="small"
        :type="activeFeature === 'deg2cart' ? 'primary' : 'default'"
        @click="onFeatureClick('deg2cart')"
        >经纬度 → 笛卡尔</n-button
      >
      <n-button
        size="small"
        :type="activeFeature === 'cart2carto' ? 'primary' : 'default'"
        @click="onFeatureClick('cart2carto')"
        >笛卡尔 → 弧度坐标</n-button
      >
      <n-button
        size="small"
        :type="activeFeature === 'screen2world' ? 'primary' : 'default'"
        @click="onFeatureClick('screen2world')"
        >点击取坐标（体验）</n-button
      >
      <n-button
        size="small"
      ...
      <n-button
        size="small"
        :type="activeFeature === 'world2screen' ? 'primary' : 'default'"
        @click="applyWorldToScreen"
        >世界 → 屏幕坐标</n-button
      >
    </div>
