...
function markDisasters() {
  ...
  const disasters = [
    { type: '地震' as const, pos: [108.95, 34.22], level: 'Ⅱ级' as const, radius: 2000, desc: '震级5.2，震源深度10km' },
    { type: '洪水' as const, pos: [108.92, 34.18], level: 'Ⅲ级' as const, radius: 1500, desc: '河道超警戒水位1.5m' },
    { type: '火灾' as const, pos: [108.97, 34.20], level: 'Ⅳ级' as const, radius: 800, desc: '林区火灾，过火面积约5公顷' },
  ];

  disasters.forEach(d => {
    const id = ++disasterIdCounter;
    const disaster: DisasterPoint = {
      id, type: d.type, position: d.pos, level: d.level, affectedRadius: d.radius, desc: d.desc,
    };
    disasterPoints.value.push(disaster);

    const levelColors: Record<string, Cesium.Color> = {
      'Ⅰ级': Cesium.Color.RED, 'Ⅱ级': Cesium.Color.ORANGE,
      'Ⅲ级': Cesium.Color.YELLOW, 'Ⅳ级': Cesium.Color.BLUE,
    };
    const color = levelColors[d.level];

    // 受灾范围（圆）
    disasterEntities.push(viewer.entities.add({
    ...
    disasterEntities.push(viewer.entities.add({
      ...
      point: { pixelSize: 18, color, outlineColor: Cesium.Color.WHITE, outlineWidth: 3 },
      label: {
        text: `${d.type}\n${d.level}`,
  ...
  viewer.flyTo(disasterEntities, { duration: 2 });
}
...
function planRescueRoutes() {
  ...
  // 救援中心
  const rescueCenter: [number, number] = [108.90, 34.25];

  // 救援中心标记
  routeEntities.push(viewer.entities.add({
    position: Cesium.Cartesian3.fromDegrees(rescueCenter[0], rescueCenter[1], 30),
    point: { pixelSize: 20, color: Cesium.Color.GREEN, outlineColor: Cesium.Color.WHITE, outlineWidth: 3 },
    label: {
      text: '应急指挥中心',
      font: 'bold 12px sans-serif',
      pixelOffset: new Cesium.Cartesian2(0, -28),
  ...
  disasterPoints.value.forEach(d => {
    ...
    const p1 = Cesium.Cartesian3.fromDegrees(rescueCenter[0], rescueCenter[1], 100);
    ...
    const route: RescueRoute = {
      id, from: rescueCenter, to: d.position, distance, estimatedTime,
    };
...
function showResources() {
  ...
  const resourceData = [
    { type: '救援队伍' as const, name: '消防救援一中队', pos: [108.91, 34.24], capacity: 50, status: '出动' as const },
    { type: '救援队伍' as const, name: '武警应急分队', pos: [108.93, 34.26], capacity: 80, status: '待命' as const },
    { type: '物资仓库' as const, name: '市级应急物资库', pos: [108.89, 34.22], capacity: 500, status: '待命' as const },
    { type: '医疗点' as const, name: '中心医院急救站', pos: [108.94, 34.23], capacity: 30, status: '出动' as const },
    { type: '避难所' as const, name: '市体育中心避难所', pos: [108.96, 34.21], capacity: 2000, status: '已满' as const },
    { type: '避难所' as const, name: '第一中学避难所', pos: [108.92, 34.19], capacity: 1500, status: '待命' as const },
  ];

  const typeColors: Record<string, string> = {
    '救援队伍': '#FF6B6B', '物资仓库': '#4ECDC4', '医疗点': '#45B7D1', '避难所': '#96CEB4',
  };
  ...
  resourceData.forEach(r => {
    ...
    const statusColor = r.status === '已满' ? Cesium.Color.RED : r.status === '出动' ? Cesium.Color.ORANGE : Cesium.Color.GREEN;

    resourceEntities.push(viewer.entities.add({
      position: Cesium.Cartesian3.fromDegrees(r.pos[0], r.pos[1], 30),
      point: { pixelSize: 14, color, outlineColor: statusColor, outlineWidth: 3 },
      label: {
        text: `${r.name}\n${r.type} | ${r.status}`,
        font: '10px sans-serif',
...
function clearAll() {
  disasterEntities.forEach(e => viewer.entities.remove(e));
  disasterEntities = [];
...
<template>
  <div class="flex h-full flex-col gap-3 p-4">
    <div class="flex flex-wrap items-center gap-2">
      <n-button size="small" type="error" @click="markDisasters">灾害标注</n-button>
      <n-button size="small" type="success" @click="planRescueRoutes">救援路径</n-button>
      <n-button size="small" type="primary" @click="showResources">资源分布</n-button>
        ...
        <div class="border-t border-gray-200 bg-gray-50 p-4">
          <div class="grid grid-cols-3 gap-4">
            ...
            <div class="rounded border border-gray-200 bg-white p-3">
              <div class="mb-2 text-sm font-semibold text-gray-700">灾害点 ({{ disasterPoints.length }})</div>
              <div v-if="disasterPoints.length === 0" class="py-2 text-center text-xs text-gray-400">暂无数据</div>
              <div v-else class="space-y-1.5">
                <div v-for="d in disasterPoints" :key="d.id" class="flex items-center gap-2 text-xs">
                  <span class="inline-block h-3 w-3 rounded-full" :style="{ background: d.level === 'Ⅰ级' ? '#ef4444' : d.level === 'Ⅱ级' ? '#f97316' : d.level === 'Ⅲ级' ? '#eab308' : '#3b82f6' }"></span>
                  <span class="font-medium">{{ d.type }}</span>
                  <n-tag size="tiny" :type="d.level === 'Ⅰ级' ? 'error' : d.level === 'Ⅱ级' ? 'warning' : d.level === 'Ⅲ级' ? 'warning' : 'info'">{{ d.level }}</n-tag>
                </div>
              </div>
            </div>
            <!-- 救援路径 -->
            <div class="rounded border border-gray-200 bg-white p-3">
              <div class="mb-2 text-sm font-semibold text-gray-700">救援路径 ({{ rescueRoutes.length }})</div>
              <div v-if="rescueRoutes.length === 0" class="py-2 text-center text-xs text-gray-400">暂无数据</div>
              <div v-else class="space-y-1.5">
                <div v-for="r in rescueRoutes" :key="r.id" class="text-xs">
                  <div class="font-medium text-green-600">路径 #{{ r.id }}</div>
                  <div class="text-gray-500">距离: {{ (r.distance / 1000).toFixed(1) }}km | 约{{ r.estimatedTime.toFixed(0) }}分钟</div>
                </div>
              </div>
            </div>
            <!-- 资源统计 -->
            <div class="rounded border border-gray-200 bg-white p-3">
              <div class="mb-2 text-sm font-semibold text-gray-700">应急资源 ({{ resources.length }})</div>
              <div v-if="resources.length === 0" class="py-2 text-center text-xs text-gray-400">暂无数据</div>
              <div v-else class="grid grid-cols-2 gap-2 text-xs">
                <div class="text-center">
                  <div class="text-lg font-bold text-red-500">{{ resources.filter(r => r.type === '救援队伍').length }}</div>
                  <div class="text-gray-500">救援队伍</div>
                </div>
                <div class="text-center">
                  <div class="text-lg font-bold text-teal-500">{{ resources.filter(r => r.type === '物资仓库').length }}</div>
                  <div class="text-gray-500">物资仓库</div>
                </div>
                <div class="text-center">
                  <div class="text-lg font-bold text-sky-500">{{ resources.filter(r => r.type === '医疗点').length }}</div>
                  <div class="text-gray-500">医疗点</div>
                </div>
                <div class="text-center">
                  <div class="text-lg font-bold text-green-500">{{ resources.filter(r => r.type === '避难所').length }}</div>
                  <div class="text-gray-500">避难所</div>
                </div>
              </div>
              <div class="mt-2 text-xs text-gray-500">
                总容量: {{ resources.reduce((s, r) => s + r.capacity, 0).toLocaleString() }} 人
              </div>
            </div>
          </div>
        </div>
