...
const statusText = ref('水利监测：点击「水库水位」查看水位监测，拖动滑块模拟水位变化。');
...
function loadReservoir() {
  ...
  // 水库范围（椭圆形）
  reservoirEntity = viewer.entities.add({
  ...
  stations.forEach(s => {
    ...
    const status = s.level >= s.warning ? '超警' : s.level >= s.warning * 0.85 ? '预警' : '正常';
    const station: MonitorStation = { id, name: s.name, position: s.pos, waterLevel: s.level, warningLevel: s.warning, status };
    monitorStations.value.push(station);

    const color = status === '超警' ? Cesium.Color.RED : status === '预警' ? Cesium.Color.ORANGE : Cesium.Color.GREEN;
  ...
  viewer.flyTo(reservoirEntity, { duration: 1.5 });
}
...
watch(waterLevel, (val) => {
  ...
  statusText.value = `当前水位：${val}m，${val >= 80 ? '超警戒水位！' : val >= 60 ? '接近警戒水位' : '正常水位范围'}`;
});
...
function loadRiver() {
  ...
  const riverPoints: [number, number][] = [
    [108.90, 34.25], [108.92, 34.23], [108.94, 34.21],
    [108.96, 34.19], [108.98, 34.17], [109.00, 34.15],
  ];
  const positions = riverPoints.map(p => Cesium.Cartesian3.fromDegrees(p[0], p[1], 5));
...
function startFloodSimulation() {
  ...
  viewer.flyTo(floodEntity, { duration: 1.5 });
}
...
function clearReservoir() {
  if (reservoirEntity) { viewer.entities.remove(reservoirEntity); reservoirEntity = null; }
...
function updateFloodPolygon(scale) {
  ...
  clear: () => `// 清除所有水利数据
if (reservoirEntity) viewer.entities.remove(reservoirEntity);
stationEntities.forEach(e => viewer.entities.remove(e));
riverEntities.forEach(e => viewer.entities.remove(e));
...
<template>
  <div class="flex h-full flex-col gap-3 p-4">
    <div class="flex flex-wrap items-center gap-2">
      <n-button size="small" type="primary" @click="loadReservoir">水库水位</n-button>
      <n-button size="small" type="success" @click="loadRiver">河道断面</n-button>
      <n-button size="small" type="warning" @click="startFloodSimulation">洪水模拟</n-button>
    ...
    <div class="flex items-center gap-4 rounded border border-gray-200 bg-gray-50 px-4 py-2 text-xs">
      ...
      <n-slider v-model:value="waterLevel" :min="0" :max="100" :step="1" style="width: 200px" />
      <span class="font-medium" :class="waterLevel >= 80 ? 'text-red-600' : waterLevel >= 60 ? 'text-orange-600' : 'text-blue-600'">{{ waterLevel }}m</span>
        ...
        <div class="border-t border-gray-200 bg-gray-50 p-4">
          <div class="mb-2 flex items-center justify-between">
            ...
            <div class="flex gap-4 text-xs">
              <span class="text-green-600">● 正常 {{ monitorStations.filter(s => s.status === '正常').length }}</span>
              <span class="text-orange-600">● 预警 {{ monitorStations.filter(s => s.status === '预警').length }}</span>
              <span class="text-red-600">● 超警 {{ monitorStations.filter(s => s.status === '超警').length }}</span>
            </div>
          </div>
          <div v-if="monitorStations.length === 0" class="py-4 text-center text-xs text-gray-400">
            点击「水库水位」加载监测站点
          </div>
          <div v-else class="grid grid-cols-3 gap-3">
            <div
              ...
              :class="s.status === '超警' ? 'border-red-300 bg-red-50' : s.status === '预警' ? 'border-orange-300 bg-orange-50' : 'border-green-300 bg-green-50'"
            >
              <div class="font-semibold" :class="s.status === '超警' ? 'text-red-700' : s.status === '预警' ? 'text-orange-700' : 'text-green-700'">{{ s.name }}</div>
              <div class="mt-1 text-gray-600">水位: <b>{{ s.waterLevel }}m</b></div>
              <div class="text-gray-600">警戒: {{ s.warningLevel }}m</div>
              <div class="mt-1"><n-tag size="small" :type="s.status === '超警' ? 'error' : s.status === '预警' ? 'warning' : 'success'">{{ s.status }}</n-tag></div>
            </div>
          </div>
        </div>
      </template>
