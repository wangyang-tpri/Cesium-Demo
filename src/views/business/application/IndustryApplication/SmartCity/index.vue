...
const statusText = ref('智慧城市：点击「加载建筑」生成城市3D建筑群，点击建筑查看详情。');
  ...
  for (let row = 0; row < 6; row++) {
    for (let col = 0; col < 6; col++) {
      ...
      const entity = viewer.entities.add({
        ...
        box: {
          dimensions: new Cesium.Cartesian3(80 + Math.random() * 40, 80 + Math.random() * 40, height),
          material: Cesium.Color.fromHsl(0.55 + Math.random() * 0.1, 0.3, 0.5 + Math.random() * 0.2),
          outline: true,
...
function loadComponents() {
  ...
  const colors: Record<string, string> = { '路灯': '#FFD700', '井盖': '#808080', '监控': '#00BFFF', '消防栓': '#FF4500' };
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const entity = viewer.entities.add({
      ...
      point: {
        pixelSize: 12,
        color: status === '正常' ? Cesium.Color.fromCssColorString(colors[type]) : Cesium.Color.RED,
  ...
  const abnormal = cityComponents.value.filter(c => c.status === '异常').length;
...
function generateHeatmap() {
  ...
  for (let row = 0; row < gridSize; row++) {
    for (let col = 0; col < gridSize; col++) {
      ...
      const dist = Math.sqrt(Math.pow(col - gridSize / 2, 2) + Math.pow(row - gridSize / 2, 2));
      const density = Math.max(0, 1 - dist / (gridSize / 1.5)) * (0.7 + Math.random() * 0.3);

      const color = density > 0.7 ? Cesium.Color.RED.withAlpha(0.6)
        : density > 0.5 ? Cesium.Color.ORANGE.withAlpha(0.6)
  ...
  statusText.value = '已生成人口热力图：红色=高密度，橙色=中高密度，黄色=中密度，绿色=低密度。';
}
  ...
  handler.setInputAction((click: any) => {
    ...
    if (picked && picked.id && picked.id.properties && picked.id.properties.buildingId) {
      ...
      if (selectedBuilding.value) {
        statusText.value = `已选中：${selectedBuilding.value.name}，${selectedBuilding.value.floors}层，高${selectedBuilding.value.height.toFixed(0)}m`;
      }
    } else {
...
<template>
  <div class="flex h-full flex-col gap-3 p-4">
    <div class="flex flex-wrap items-center gap-2">
      <n-button size="small" type="primary" @click="generateBuildings">加载建筑</n-button>
      <n-button size="small" type="success" @click="loadComponents">城市部件</n-button>
      <n-button size="small" type="warning" @click="generateHeatmap">人口热力</n-button>
      ...
      <template #bottom-panel>
        <div class="border-t border-gray-200 bg-gray-50 p-4">
          <div class="grid grid-cols-2 gap-4">
            <!-- 建筑详情 -->
            <div v-if="selectedBuilding" class="rounded border border-blue-200 bg-blue-50 p-3">
              <div class="mb-2 text-sm font-semibold text-blue-700">建筑详情</div>
              <div class="grid grid-cols-2 gap-2 text-xs">
                <div><span class="text-gray-500">名称：</span>{{ selectedBuilding.name }}</div>
                <div><span class="text-gray-500">类型：</span>{{ selectedBuilding.type }}</div>
                <div><span class="text-gray-500">高度：</span>{{ selectedBuilding.height.toFixed(0) }}m</div>
                <div><span class="text-gray-500">楼层：</span>{{ selectedBuilding.floors }}层</div>
                <div class="col-span-2"><span class="text-gray-500">坐标：</span>{{ selectedBuilding.position[0].toFixed(6) }}, {{ selectedBuilding.position[1].toFixed(6) }}</div>
              </div>
            </div>
            <!-- 统计信息 -->
            <div class="rounded border border-gray-200 bg-white p-3">
              <div class="mb-2 text-sm font-semibold text-gray-700">城市数据统计</div>
              <div class="grid grid-cols-3 gap-2 text-center text-xs">
                <div>
                  <div class="text-lg font-bold text-blue-600">{{ buildings.length }}</div>
                  <div class="text-gray-500">建筑数量</div>
                </div>
                <div>
                  <div class="text-lg font-bold text-green-600">{{ cityComponents.length }}</div>
                  <div class="text-gray-500">部件数量</div>
                </div>
                <div>
                  <div class="text-lg font-bold text-red-600">{{ cityComponents.filter(c => c.status === '异常').length }}</div>
