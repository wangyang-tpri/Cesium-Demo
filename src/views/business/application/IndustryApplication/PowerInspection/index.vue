...
function loadTransmissionLine() {
  ...
  setupPickHandler();
  statusText.value = `已加载输电线路，共 ${towers.value.length} 基杆塔，${
    towers.value.filter((t) => t.status === "故障").length
  } 基故障。点击杆塔查看详情。`;
...
function showCorridor() {
  ...
  for (let i = 0; i < towers.value.length; i++) {
    corridorPositions.push(
      Cesium.Cartesian3.fromDegrees(
        towers.value[i].position[0] - halfWidth,
        towers.value[i].position[1],
        0
      )
    );
  }
  ...
  statusText.value =
    "已显示线路走廊安全区（黄色半透明区域，约300米宽）。走廊内禁止违规建筑和超高树木。";
}
...
function markFaults() {
  ...
  const faultTypes: FaultPoint["type"][] = [
    "树障",
    "异物",
    "绝缘子破损",
    "导线断股",
...
function clearAll() {
  ...
  if (v) {
    ...
    if (corridorEntity) {
      v.entities.remove(corridorEntity);
      corridorEntity = null;
    }
  }
  towerEntities = [];
...
<template>
  <div class="flex h-full flex-col gap-3 p-4">
    <div class="flex flex-wrap items-center gap-2">
      <n-button size="small" type="primary" @click="loadTransmissionLine"
        >输电线路</n-button
      >
      <n-button size="small" type="success" @click="showCorridor"
        >走廊分析</n-button
      >
      <n-button size="small" type="warning" @click="markFaults"
        >故障标记</n-button
      >
            ...
            >
              ...
              <div class="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  ...
                  >{{ selectedTower.height.toFixed(0) }}m
                </div>
                <div>
                  ...
                  <n-tag
                    size="small"
                    :type="
                      selectedTower.status === '故障'
                        ? 'error'
                        : selectedTower.status === '巡检中'
                        ? 'warning'
                        : 'success'
                    "
                    >{{ selectedTower.status }}</n-tag
                  >
                </div>
              </div>
            </div>
              ...
              <div class="grid grid-cols-4 gap-2 text-center text-xs">
                ...
                <div>
                  <div class="text-lg font-bold text-green-600">
                    {{ towers.filter((t) => t.status === "正常").length }}
                  </div>
                  <div class="text-gray-500">正常</div>
                </div>
                <div>
                  <div class="text-lg font-bold text-orange-600">
                    {{ towers.filter((t) => t.status === "巡检中").length }}
                  </div>
                  <div class="text-gray-500">巡检中</div>
                </div>
                <div>
                  <div class="text-lg font-bold text-red-600">
                    {{ towers.filter((t) => t.status === "故障").length }}
                  </div>
                  <div class="text-gray-500">故障</div>
                </div>
              </div>
              <div
                v-if="faultPoints.length > 0"
                class="mt-2 text-xs text-gray-500"
              >
                故障点: {{ faultPoints.length }} 个（紧急{{
                  faultPoints.filter((f) => f.level === "紧急").length
                }}
                / 严重{{
                  faultPoints.filter((f) => f.level === "严重").length
                }}
                / 一般{{
                  faultPoints.filter((f) => f.level === "一般").length
                }}）
              </div>
            </div>
          </div>
