...
function startPick(type: PointType) {
  ...
  currentPick = type;
  statusText.value =
    type === "observer" ? "请在地图上点击观察点..." : "请在地图上点击目标点...";
  if (!handler) {
    const v = viewer.value!;
...
async function analyzeLineOfSight() {
  ...
  if (blockIndex >= 0 && updatedPositions[blockIndex]) {
    const blockPos = Cesium.Cartographic.toCartesian(
      updatedPositions[blockIndex]
    );
