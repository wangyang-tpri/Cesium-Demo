...
const statusText = ref("剖面分析：在地图上点击添加剖面线顶点，点击「完成」生成高程剖面图。");
...
function initChart() {
  ...
  chartInstance.setOption({
    title: { text: "地形高程剖面", left: "center", textStyle: { fontSize: 14 } },
    tooltip: { trigger: "axis", formatter: (params: any) => {
      const p = params[0];
      return `距离: ${p.value[0].toFixed(0)} m<br/>高程: ${p.value[1].toFixed(1)} m`;
    }},
    grid: { left: 60, right: 20, top: 50, bottom: 40 },
    xAxis: { type: "value", name: "距离 (m)", nameLocation: "middle", nameGap: 25 },
    yAxis: { type: "value", name: "高程 (m)", nameLocation: "middle", nameGap: 45 },
    series: [{
      type: "line",
      smooth: true,
      symbol: "none",
...
function clearAll() {
  ...
  pointEntities = [];
  if (lineEntity) { v.entities.remove(lineEntity); lineEntity = null; }
  if (profileLineEntity) { v.entities.remove(profileLineEntity); profileLineEntity = null; }
  points = [];
...
function startPicking() {
  clearAll();
  picking = true;
  statusText.value = "请在地图上点击添加剖面线顶点（至少2个点），点击「完成」生成剖面...";
  if (!handler) {
    const v = viewer.value!;
    ...
    handler.setInputAction((click: any) => {
      ...
      const entity = v.entities.add({
        position: pos,
        point: { pixelSize: 10, color: Cesium.Color.fromCssColorString("#2080f0"), outlineColor: Cesium.Color.WHITE, outlineWidth: 2 },
        label: { text: `P${points.length}`, font: "12px sans-serif", pixelOffset: new Cesium.Cartesian2(0, -20), fillColor: Cesium.Color.WHITE, outlineColor: Cesium.Color.BLACK, outlineWidth: 2, style: Cesium.LabelStyle.FILL_AND_OUTLINE },
      });
      ...
      if (points.length >= 2) {
        lineEntity = v.entities.add({
          polyline: { positions: [...points], width: 2, material: Cesium.Color.fromCssColorString("#2080f0").withAlpha(0.6), dashPattern: 255 },
        });
      }
...
async function finishPicking() {
  ...
  // 最后一个点
  cartographics.push(Cesium.Cartographic.fromCartesian(points[points.length - 1]));
  ...
  // 构建 ECharts 数据
  const chartData = distances.map((d, i) => [d, updatedPositions[i]?.height ?? 0]);
  ...
  // 在地图上绘制剖面线（用实际地形高度）
  const profilePositions = updatedPositions.map((c) => Cesium.Cartographic.toCartesian(c));
  if (profileLineEntity) v.entities.remove(profileLineEntity);
  profileLineEntity = v.entities.add({
    polyline: { positions: profilePositions, width: 5, material: new Cesium.PolylineGlowMaterialProperty({ glowPower: 0.25, color: Cesium.Color.fromCssColorString("#2080f0") }) },
  });
  ...
  statusText.value = `✅ 剖面生成完成：总长 ${(cumulativeDist / 1000).toFixed(2)} km | 最高 ${maxH.toFixed(0)}m | 最低 ${minH.toFixed(0)}m | 平均 ${avgH.toFixed(0)}m`;
}
