    ...
    (e: Cesium.ScreenSpaceEventHandler.PositionedEvent) => {
      ...
      if (mode === "pick-entity") {
        ...
        if (Cesium.defined(picked) && picked.id instanceof Cesium.Entity) {
          ...
          const labelText =
            text instanceof Cesium.ConstantProperty
              ? String(text.getValue(Cesium.JulianDate.now()))
              : text;
          statusText.value = `scene.pick → 命中实体【${String(picked.id.id)}】${
            labelText ? `（${labelText}）` : ""
          }`;
        } else {
        ...
        if (Cesium.defined(cartesian)) {
          ...
          statusText.value =
            `scene.pickPosition → 表面坐标：经度 ${Cesium.Math.toDegrees(
              carto.longitude
            ).toFixed(5)}°，` +
          ...
          statusText.value =
            "scene.pickPosition → 未命中（天空/深度纹理不可用）";
        }
        ...
        if (cartesian) {
          ...
          statusText.value =
            ...
            `纬度 ${Cesium.Math.toDegrees(carto.latitude).toFixed(
              5
            )}°（无视模型高度）`;
        }
        ...
        if (hits.length) {
          const names = hits
            .map(
              (h, i) =>
                `${i + 1}.${
                  h.id && h.id.id !== undefined
                    ? String(h.id.id)
                    : h.primitive?.constructor.name ?? "?"
                }`
            )
            .join("  ");
...
“pick 实体 + pickPosition 取点”的组合方案。`,
  ...
  "pick-ellipsoid":
    () => `【原理】camera.pickEllipsoid 从相机位置向屏幕点发射射线，
...
<template>
  <div class="flex h-full flex-col gap-3 p-4">
    <div class="flex flex-wrap items-center gap-2">
      <n-button
        size="small"
        :type="activeFeature === 'pick-entity' ? 'primary' : 'default'"
        @click="setMode('pick-entity')"
        >拾取实体</n-button
      >
      <n-button
        size="small"
        :type="activeFeature === 'pick-position' ? 'primary' : 'default'"
        @click="setMode('pick-position')"
        >深度拾取</n-button
      >
      <n-button
        size="small"
        :type="activeFeature === 'pick-ellipsoid' ? 'primary' : 'default'"
        @click="setMode('pick-ellipsoid')"
        >椭球面拾取</n-button
      >
      <n-button
        size="small"
        :type="activeFeature === 'drill-pick' ? 'primary' : 'default'"
        @click="setMode('drill-pick')"
        >穿透拾取</n-button
      >
    </div>
