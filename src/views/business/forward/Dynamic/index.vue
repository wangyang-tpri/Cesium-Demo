...
const statusText = ref('动态效果：基于 CallbackProperty 随时间变化的视觉表达。');
...
function seconds(time: Cesium.JulianDate | undefined): number {
  ...
  return Cesium.JulianDate.secondsDifference(time ?? Cesium.JulianDate.now(), Cesium.JulianDate.fromIso8601('2026-01-01T00:00:00Z'));
}
...
function applyRipple() {
      ...
      ellipse: {
        ...
        material: new Cesium.ColorMaterialProperty(
          new Cesium.CallbackProperty((time) => {
            ...
            return Cesium.Color.fromCssColorString('#00e676').withAlpha(0.6 * (1 - t));
          }, false)
        ),
        height: 0,
      },
  ...
  statusText.value = '扩散波纹：半径 4 秒一循环（CallbackProperty 驱动椭圆与透明度）';
}
  ...
  entities.push(
    ...
    v.entities.add({
      position: new Cesium.CallbackProperty((time) => {
        ...
        const i = Math.floor(idx);
        const f = idx - i;
        return Cesium.Cartesian3.lerp(path[i]!, path[Math.min(i + 1, path.length - 1)]!, f, new Cesium.Cartesian3());
      }, false) as unknown as Cesium.PositionProperty,
      point: {
        pixelSize: 18,
        color: new Cesium.CallbackProperty((time) => {
          const f = (seconds(time) % 0.5) / 0.5;
          return Cesium.Color.fromCssColorString('#ffeaa7').withAlpha(1 - f * 0.7);
        }, false),
      },
    })
  );
...
高频变化的属性务必传 false；不变的函数传 true 可省计算。`,
  ...
  ripple: () => `【原理】波纹 = 半径扩展 + 透明度衰减两个 CallbackProperty 叠加。
比 Entity 动画（position 移动）更轻量：只更新 uniform/几何参数。

【要点】同一 time 函数的不同相位（% 4 与 /4）分别控制
形状与颜色的节奏，可做出“多层波纹”效果。`,

  arrow: () => `【原理】PolylineArrowMaterialProperty 把线渲染为连续箭头（表达流向），
...
<template>
  <div class="flex h-full flex-col gap-3 p-4">
    <div class="flex flex-wrap items-center gap-2">
      <n-button size="small" :type="activeFeature === 'glow-line' ? 'primary' : 'default'" @click="applyGlowLine">流动光点</n-button>
      <n-button size="small" :type="activeFeature === 'ripple' ? 'primary' : 'default'" @click="applyRipple">扩散波纹</n-button>
      <n-button size="small" :type="activeFeature === 'arrow' ? 'primary' : 'default'" @click="applyArrow">动态箭头</n-button>
      <n-button size="small" :type="activeFeature === 'scan' ? 'primary' : 'default'" @click="applyScan">扫描圈</n-button>
