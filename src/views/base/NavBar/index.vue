        ...
        children: [
          {
            label: "坐标拾取",
            key: "coordinate-pick",
            icon: renderIcon(PickIcon),
          },
          {
            label: "图元绘制",
            key: "primitive-draw",
            icon: renderIcon(DrawIcon),
          },
          { label: "标注工具", key: "annotation", icon: renderIcon(AnnoIcon) },
          {
            label: "图层切换",
            key: "layer-switch",
            icon: renderIcon(LayerSwitchIcon),
          },
        ],
      },
      {
...
onMounted(async () => {
  ...
  const group = menuOptions.find((g) =>
    g.children?.some((c) => c.key === name)
  );
  if (group) {
  ...
  for (const g of menuOptions) {
    const subGroup = g.children?.find((c: any) =>
      c.children?.some((cc: any) => cc.key === name)
    );
    if (subGroup) {
